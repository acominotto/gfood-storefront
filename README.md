...
4. **Place order** — `POST /wc/store/v1/checkout` with billing and shipping addresses, `payment_method`, optional `customer_note` / `payment_data`. The storefront stays on its thank-you page; if Woo returns `payment_result.redirect_url`, that URL is offered as a **link** to g-food.ch (new tab) instead of replacing the page. Set `NEXT_PUBLIC_WOO_SITE_URL` (same as `WP_BASE_URL`) so COD-style flows without a redirect can still link to `/checkout/order-received/…` when `order_key` is present. **Note:** payment methods that require the customer to **complete payment** on the redirect URL (e.g. hosted checkout) will not work until you restore an automatic redirect for those gateways.
