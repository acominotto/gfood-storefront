...
import { wooGet, wooPost } from "@/server/woo-request";

export async function GET() {
  try {
    const response = await wooGet("cart");
    const cart = cartResponseSchema.parse(await response.json());
    return jsonOk(cart);
  } catch (error) {
    return jsonError(400, error instanceof Error ? error.message : "Unable to load cart");
  }
}
